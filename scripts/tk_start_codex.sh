...
if codex --help 2>/dev/null | grep -q "dangerously-bypass-approvals-and-sandbox"; then
  exec codex --dangerously-bypass-approvals-and-sandbox -- "$BOOT_PROMPT"
else
  exec codex --full-auto -- "$BOOT_PROMPT"
fi
