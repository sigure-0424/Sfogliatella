...
class RNNModel(eqx.Module):
    """Stacked vanilla RNN with tanh activations."""
    cells: List[eqx.nn.GRUCell]   # using GRU as a cleaner vanilla-style RNN
    ...
    def __init__(
        ...
        *,
        ...
        for i in range(n_layers):
            ...
            cells.append(eqx.nn.GRUCell(input_size=cell_in, hidden_size=hidden, key=keys[i]))
        self.cells = cells
    ...
    def __call__(self, x: jnp.ndarray, *, key: Optional[jax.Array] = None) -> jnp.ndarray:
        seq_len = x.shape[0]
        states = [jnp.zeros(self.hidden) for _ in self.cells]
...
@register("rnn")
def build_rnn(config: Dict[str, Any], rng_key=None, sample_x=None):
    import jax
    ...
    return RNNModel(input_dim, out_dim, hidden=hidden, n_layers=n_layers, key=key)
